...
def get_file_list(folder_path: str, p_postfix: str or list = ['.png'], sub_dir: bool = True) -> list:
    """
...
class Pytorch_model:
    ...
    def predict_cv(self, img: np.ndarray or str):
        ...
        t_img = cv2.resize(img, (self.img_w, self.img_h)) / 255.
        print(t_img.shape)
        # 将图片由(w,h)变为(1,img_channel,h,w)
        ...
        tensor = tensor.to(self.device)
        preds = self.net(tensor)
        label = torch.from_numpy(np.load('crop/22_1 copy.png.npy'))
        # preds = preds[0].permute(1, 2, 0).detach().cpu().numpy()
        preds = preds[0].permute(1, 2, 0).detach().cpu().numpy()
        print(label.permute(1, 2, 0))
        print(np.abs(preds))
        unwarp_img = cv2.remap(t_img, np.abs(preds[:, :, 0]), np.abs(preds[:, :, 1]), cv2.INTER_LINEAR)
        unwarp_img = cv2.resize(unwarp_img, (w, h))
...
if __name__ == '__main__':
    ...
    # plt.imshow(img)
    # print(unwarp_img)
    # print(unwarp_img * 255)
    # cv2.imshow('crop', unwarp_img * 255)
    cv2.imwrite('./temp/temp.png', unwarp_img * 255)
    # plt.subplot(1, 2, 2)
    ...
    # a = np.load('crop/1_1 copy.png.npy')
    # print(a)
