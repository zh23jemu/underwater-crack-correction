...
def build_rows(specs: Iterable[ExperimentSpec]) -> List[Dict[str, object]]:
    ...
    for spec in specs:
        ...
        # compare_summary.py 中 old 是对比项，new 是 v4；positive 表示 v4 综合分更好。
        row["v4_better_images"] = int(compare.get("num_new_score_positive", 0))
        row["other_better_images"] = int(compare.get("num_new_score_negative", 0))
        rows.append(row)
